...
RSpec.describe CatalogController do
  ...
  describe "#index" do
    ...
    let(:document) do
      {:id => "good", :reviewed_ssim => "true", :desc_metadata__set_sim => "http://oregondigital.org/resource/oregondigital:building-or", :desc_metadata__latitude_teim => "9001", :desc_metadata__type_label_sim => "Image$http://purl.org/dc/dcmitype/Image"}
    end
    ...
    context "when there are documents that are non image ones" do
      before do
        solr.add(document)
        solr.add(bad_document(:desc_metadata__type_label_sim => "PDF$http://Blah.com"))
        solr.commit
        get :index
      end
